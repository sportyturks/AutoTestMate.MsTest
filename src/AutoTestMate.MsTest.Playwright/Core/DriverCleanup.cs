	...
	public class DriverCleanup : IDriverCleanup
	{
		...
		private void CleanBrowserProcesses(bool onInitialise = false)
		{
			try
			{
				...
				if (string.Equals(Infrastructure.Constants.Generic.TrueValue.ToLower(), _configurationReader.GetConfigurationValue(Constants.Configuration.ForceKillProcessKey).ToLower()))
				{
